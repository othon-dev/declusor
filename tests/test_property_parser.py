...
class TestParserProperties(unittest.TestCase):
    ...
    @given(st.text())
    def test_parse_never_crashes_on_random_input(self, input_str):
        """
    ...
    @given(st.dictionaries(keys=st.text(min_size=1, max_size=10, alphabet=st.characters(whitelist_categories=('L', 'N'))), 
                           values=st.text(min_size=1, max_size=20).filter(lambda x: not x.startswith("-"))))
    def test_parse_valid_input(self, args_dict):
        """
